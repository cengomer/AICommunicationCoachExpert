...
struct ImagePicker: View {
    ...
    var body: some View {
        GeometryReader {
            ...
            .contentShape(.rect)
            /// Implementing Drop Actions and Retreving Dropped Image
        ...
        Task.detached {
            ...
            await MainActor.run {
                if let selectedImage = UIImage(data: imageData) {
                    ...
                    /// Send Orginal Image to Callback
...
extension View {
    @ViewBuilder
    func optionalViewModifier<Content: View>(@ViewBuilder content: @escaping (Self) ->
                                             Content) -> some View {
        content(self)
    }
}
